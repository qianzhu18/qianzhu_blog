...
function isWrappedRecordValue(value) {
  return Boolean(
    value &&
      typeof value === 'object' &&
      !Array.isArray(value) &&
...
function normalizeRecordEntry(entry) {
  ...
  for (let i = 0; i < 5; i++) {
    if (!isWrappedRecordValue(normalized.value)) {
      break
    }

    const wrappedValue = normalized.value
    normalized = {
      ...normalized,
      role: normalized.role ?? wrappedValue.role,
...
function normalizeRecordMap(recordMap) {
  ...
  const normalizedRecordMap = { ...recordMap }

  ;['block', 'collection', 'collection_view', 'notion_user'].forEach(table => {
    const tableData = normalizedRecordMap[table]
    if (!tableData || typeof tableData !== 'object') {
      return
    }

    normalizedRecordMap[table] = Object.fromEntries(
      Object.entries(tableData).map(([id, entry]) => [id, normalizeRecordEntry(entry)])
    )
  })
